...
export class RankingService {
   ...
   */
  private calculateCGPAScore(cgpa: number, maxCGPA: number, percentile: number): number {
  ...
  private calculateWAECScore(grades: string[]): number {
    ...
    const sumPoints = grades.reduce((sum, grade) => sum + (gradePoints[grade] || 0), 0);
  ...
  private calculateCompetitionScore(participations: any[]): number {
    return participations.reduce((total, p) => {
      ...
      if (position.includes('national') || position.includes('international')) {
        if (position.includes('1st') || position.includes('winner') || position.includes('champion')) {
          return total + 10;
        }
  ...
  async calculateStudentScore(studentId: string): Promise<ScoreBreakdown> {
    ...
    const [academicRecords, extracurriculars, eventParticipations] = await Promise.all([
    ...
    const waecRecords = academicRecords.filter(r => 
      r.institution.type?.toLowerCase().includes('waec') || 
      r.institution.type?.toLowerCase().includes('neco')
    );
    const cgpaRecords = academicRecords.filter(r => 
      r.institution.type?.toLowerCase().includes('university') || 
      r.institution.type?.toLowerCase().includes('tertiary')
    );
    const schoolRecords = academicRecords.filter(r => 
      !jambRecords.includes(r) && !waecRecords.includes(r) && !cgpaRecords.includes(r)
    );
    ...
    if (waecRecords.length > 0) {
      const sittingsBySession = waecRecords.reduce((acc, record) => {
      ...
      const sittingScores = Object.values(sittingsBySession).map(grades => 
        this.calculateWAECScore(grades)
      );
      examScore = Math.max(examScore, ...sittingScores);
    }
    ...
    if (cgpaRecords.length > 0) {
      for (const record of cgpaRecords) {
        ...
        cgpaScore = Math.max(cgpaScore, this.calculateCGPAScore(cgpa, maxCGPA, percentile));
      }
    }
    ...
    if (schoolRecords.length > 0) {
      for (const record of schoolRecords) {
        ...
        schoolGradesScore = Math.max(schoolGradesScore, this.calculateSchoolGradesScore(percentile));
      }
    }
    ...
    const recognizedParticipations = eventParticipations.filter(p => p.event.isRecognized);
    const competitionScore = this.calculateCompetitionScore(recognizedParticipations);

    const totalScore = schoolGradesScore + cgpaScore + examScore + extracurricularScore + competitionScore;
  ...
  async getMyPosition(studentId: string) {
    ...
    return {
      ...myRank,
      totalStudents: allRankings.length,
      percentile: ((allRankings.length - myRank.rank + 1) / allRankings.length) * 100
    };
  }
}
